...
### DATA SECTION ###
#########################################################################
data <- read.csv("reddrumCKM_processed_newrec.csv", stringsAsFactors=FALSE)     #reading in the raw files output from SAS 
...
ckm_yrly <-cbind(meancpue, varcpue$bothvar)
head(ckm_yrly)
...
write.csv(ckm_yrly, file = "ckm_annual_recs_newrec.csv", row.names=FALSE)
