...
### DATA SECTION ###
#########################################################################
data <- read.csv("reddrumTBM_processed_newrec.csv", stringsAsFactors=FALSE)     #reading in the raw files output from SAS 
...
tbm_yrly <-cbind(meancpue, varcpue$bothvar)
head(tbm_yrly)
...
write.csv(tbm_yrly, file = "tbm_annual_recs_newrec.csv", row.names=FALSE)
