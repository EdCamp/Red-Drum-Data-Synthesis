...
### DATA SECTION ###
#########################################################################
data <- read.csv("reddrumCHM_processed_newrec.csv", stringsAsFactors=FALSE)     #reading in the raw files output from SAS 
...
dat$rec_cpue <- dat$rec/dat$effort 

#subsetting out for only the columns you think you'll use
dat1 <- dat[,c("gear", "effort", "zone", "month", "tide", "yoy_cpue", "rec_cpue", "year", "yr_yoy", "bot_x", "bveg_x", "shore_x", "temp", "sal")]
...
with(d, tapply(rec_cpue, list(year, month), sum))   #Adequate samples in years 1998:2015

#yr_yoy--back-assigned fish caught in first  have of year x to previous year (jan:aug 2015 catch was born in 2014)
with(d, tapply(rec_cpue, list(yr_yoy, month), sum))   #Remember, yoy caught in Jan:Jul 2015 get classed as 2014, so while plenty of fish caught in 2015, only the ones after august get counted for that year.
with(d, tapply(rec_cpue, list(yr_yoy, zone), sum))    #Pretty balanced for all zones
# with(d, tapply(rec_cpue, list(yr_yoy, bot_x), sum))   #Unknown is spotty, but currently not standardizing by this.
# with(d, tapply(rec_cpue, list(yr_yoy, bveg_x), sum))  #Well-balanced
# with(d, tapply(rec_cpue, list(yr_yoy, shore_x), sum)) #Well-balanced
with(d, tapply(rec_cpue, list(yr_yoy, gear), sum))    #gears are good

#editing data according to above
d <- d[which(d$zone %in% c("A", "B", "C", "D", "M", "P")),]    
d$zone <- as.factor(as.character(d$zone))
levels(d$zone)

#d <- d[which(!d$yr_yoy =="1995"),]
#d$yr_yoy <- as.factor(as.character(d$yr_yoy))
#levels(d$yr_yoy)

### SPLITTING DATA FOR DELTA LOG NORMAL
#Splitting data for delta log normal
posdata <- d[which(d$rec_cpue>0),]                      #these are the counts of the positive samples
padata <- d                                           #replicate data set
padata$rec_cpue[which(padata$rec_cpue>0)]=1           #this for the binomial, replace positives with 1 to make binomial catch
...
binom.glm.res=glm(rec_cpue ~ zone + yr_yoy + month + gear, data=padata,family="binomial")   #positive catch, yes or no
ln.glm.res=glm(log(rec_cpue) ~ zone + yr_yoy + month + gear, data=posdata)                  #log normal catch...which seems like another distribution could have been used.
summary(ln.glm.res)
...
chm_yrly <-cbind(meancpue, varcpue$bothvar)
head(chm_yrly)
...
write.csv(chm_yrly, file = "chm_annual_recs_newrec.csv", row.names=FALSE)
