...
with(d, tapply(yoy_cpue, list(yr_yoy, zone), sum))    #E, G, & H are not well sampled through time
# with(d, tapply(yoy_cpue, list(yr_yoy, bot_x), sum))   #Unknown could be dropped I guess
...
d <- d[which(d$zone %in% c("A", "B", "C", "D", "M", "P")),]         #No need to take out any zones, and remember that the letters differ by site
...
both=new.binom$fit*exp(new.ln$fit)                             #Original from Rob
#both=(new.binom$fit/(1-new.binom$fit))*exp(new.ln$fit)          #Shouldn't we inverse logit transform the binomial estimates?
...
bothvar=new.binom$se.fit^2*new.ln$se.fit^2+new.binom$se.fit^2*mean(exp(new.ln$fit))^2+new.ln$se.fit^2*mean(new.binom$fit)^2   #Shouldn't E(ln)^2 be mean(exp(ln_fit))^2??
...
write.csv(chm_yrly, file = "chm_annual_all_yoys_newrec.csv", row.names=FALSE)
