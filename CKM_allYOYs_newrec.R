...
# with(d, tapply(yoy_cpue, list(yr_yoy, bot_x), sum))   #These are all fine
...
#d <- d[which(d$zone %in% c("A", "B", "C")),]         #No need to take out any zones, and remember that the letters differ by site
...
write.csv(ckm_yrly, file = "ckm_annual_all_yoys_newrec.csv", row.names=FALSE)
